...
import { Zap, Shield, Crosshair, Activity, Bot, User, ArrowUpCircle, ArrowDownCircle, ArrowRightLeft, BrainCircuit } from "lucide-react";
...
export default function StrategyTuner() {
      ...
      try {
        ...
        if (data.settings) {
          setConfig({
            ...
            direction: data.settings.strategy?.direction || "both"
          });
        }
      } catch (error) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12 p-1">
      ...
      {/* 1. EXECUTION LOGIC (Compact) */}
      <Card className="bg-zinc-950 border-zinc-800">
        <div className="flex items-center justify-between p-4">
            <div className="flex items-center gap-4">
                <div className={`p-2.5 rounded-full ${config.executionMode === 'auto' ? 'bg-red-500/10 text-red-500' : 'bg-blue-500/10 text-blue-400'}`}>
                    {config.executionMode === 'auto' ? <Zap size={20} fill="currentColor" /> : <Bot size={20} />}
                </div>
                <div>
                    <h3 className="text-sm font-semibold text-zinc-100 flex items-center gap-2">
                        Execution Logic
                        <Badge variant="outline" className={`ml-2 border text-[10px] uppercase font-bold tracking-wider h-5 px-1.5 ${config.executionMode === 'auto' ? 'border-red-500/30 text-red-400 bg-red-500/5' : 'border-blue-500/30 text-blue-400 bg-blue-500/5'}`}>
                            {config.executionMode === 'auto' ? 'Fully Automated' : 'Manual Confirm'}
                        </Badge>
                    </h3>
                    <p className="text-xs text-zinc-500 mt-0.5 max-w-lg">
                        {config.executionMode === 'auto' 
                            ? "Bot executes orders instantly upon signal generation." 
                            : "Bot waits for your approval before placing orders."}
                    </p>
                </div>
            </div>

            <div className="flex items-center gap-3">
                <Label htmlFor="auto-mode" className="text-xs text-zinc-400 font-medium">Auto Mode</Label>
                <Switch 
                    id="auto-mode"
      ...
      {/* 3. MARKET BIAS (Guardrails) */}
      <Card className="bg-zinc-950 border-zinc-800">
        <CardHeader>
            <CardTitle className="text-base text-zinc-200 flex items-center gap-2">
                <Crosshair size={18} className="text-zinc-400" /> Market Bias (Direction)
            </CardTitle>
            <CardDescription>Restrict the bot to specific trade directions.</CardDescription>
        </CardHeader>
        <CardContent>
            <div className="flex flex-wrap gap-3">
                {[
                    { id: "both", label: "Bi-Directional", icon: ArrowRightLeft, color: "text-zinc-100" },
                    { id: "long", label: "Long Only", icon: ArrowUpCircle, color: "text-emerald-400" },
                    { id: "short", label: "Short Only", icon: ArrowDownCircle, color: "text-red-400" },
                ].map((mode) => (
                    <Button
                        key={mode.id}
                        variant="outline"
                        onClick={() => handleUpdate("direction", mode.id)}
                        className={`
                            h-11 px-5 gap-2.5 border transition-all text-sm
                            ${config.direction === mode.id 
                                ? `bg-zinc-800 border-zinc-600 text-white shadow-inner` 
                                : `bg-transparent border-zinc-800 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900 hover:border-zinc-700`}
                        `}
                    >
                        <mode.icon size={16} className={config.direction === mode.id ? mode.color : "text-zinc-600"} />
                        {mode.label}
                    </Button>
                ))}
            </div>
