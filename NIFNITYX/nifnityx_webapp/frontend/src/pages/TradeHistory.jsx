...
export default function TradeHistory() {
  ...
  const filteredTrades = trades.filter(trade => {
    ...
    else if (activeTab === "history") matchesTab = ["WIN", "LOSS"].includes(trade.status);
    else if (activeTab === "logs") matchesTab = ["REJECTED", "PENDING_APPROVAL"].includes(trade.status);

    let matchesMode = true;
    if (modeFilter === "live") matchesMode = trade.execution_mode === "LIVE";
    if (modeFilter === "paper") matchesMode = trade.execution_mode === "PAPER";

    return matchesSearch && matchesTab && matchesMode;
  });
  ...
  const hasActiveFilters = activeTab !== 'all' || modeFilter !== 'all' || searchQuery !== '';
  ...
  const ModeBadge = ({ mode }) => {
    if (mode === "LIVE") {
      return (
        <Badge variant="secondary" className="bg-amber-500/5 text-amber-400/80 border-amber-500/10 text-[9px] h-5 px-1.5">
          <Zap size={9} className="mr-1" /> LIVE
        </Badge>
      );
    }
    return (
      <Badge variant="secondary" className="bg-indigo-500/5 text-indigo-400/80 border-indigo-500/10 text-[9px] h-5 px-1.5">
      ...
      <div className="flex-none flex flex-col xl:flex-row gap-3 items-start xl:items-center justify-between bg-zinc-900/30 p-2 rounded-xl border border-zinc-800/50 backdrop-blur-sm">
        <Tabs value={activeTab} className="w-full xl:w-auto" onValueChange={setActiveTab}>
          <TabsList className="bg-zinc-950 border border-zinc-800 h-8 p-0.5">
            ...
            <TabsTrigger value="live" className="text-[11px] px-3 data-[state=active]:bg-blue-500/10 data-[state=active]:text-blue-400 text-zinc-500">Open</TabsTrigger>
            ...
            <table className="w-full text-sm text-left caption-bottom">
              <TableHeader className="sticky top-0 z-20 bg-zinc-950 border-b border-zinc-800 shadow-sm">
                <TableRow className="border-none hover:bg-transparent">
                  ...
                  <TableHead className="text-zinc-400 font-medium bg-zinc-950">Mode</TableHead>
                ...
                {loading ? (
                  ...
                  filteredTrades.map((trade) => (
                    <TableRow key={trade._id} className="border-b border-zinc-800/40 hover:bg-zinc-900/40 transition-colors group">
                      ...
                      <TableCell className="align-top py-3">
                        <ModeBadge mode={trade.execution_mode} />
                      </TableCell>
